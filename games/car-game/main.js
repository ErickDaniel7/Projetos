...
function app() {
	...
	class Game {
		constructor(args) {
			// 0: facil, 1: medio (padrão), 2: dificil, 3: brutal
	...
	class Vehicle {
		constructor(args) {
			...
			// 0: caro, 1: caminhão, 2: trailer
			let modelID = args.modelID;

			if (modelID === undefined)
				modelID = Math.round(Math.random() * 2)

			// as dimensões referem-se ao hitbox
			switch (modelID) {
				case 1:
					this.width = 5;
					this.height = 5;
		...
		move() {
			...
			// falha automática se fora dos limites
	...
	class Spark {
		...
		moveParticles() {
			// encolher partículas à medida que se movem
	...
	var init = () => {
		...
		checkCollision = (a,b) => {
			if (!a.crashed && !b.crashed && (a.name != b.name)) {
				...
				// direita acerta esquerda, esquerda acerta direita
				let touchedX_RL = (A_left <= B_right && A_left >= B_left),
					touchedX_LR = (A_right >= B_left && A_right <= B_right),
				// a frente bate nas costas, as costas batem na frente
				...
				if ((touchedX_RL || touchedX_LR) && (touchedZ_FB || touchedZ_BF)) {
					...
					// x-center a faísca entre as extremidades que se chocam
					if (A_left <= B_right)
						sx = (A_left + B_right)/2;
					...
					// faísca de gatilho para o veículo do jogador
					if (a.name == "Vehicle 0") {
						a.crashed = true;
						game.over = true;
		...
		startGame = difficulty => {
			...
			// primeira vez ou começando de novo
			if (game == null || game.over) {
				game = new Game({
			...
			if (game != null && cameraY == cameraYMin) {
				...
				if (vehicleCtrld.z <= -firstChunkSize) {
					...
					for (let v of vehiclesBehind) {
						...
						// marque pontos por passar sem usar o ombro
		...
		steerVehicle = e => {
			if (game != null && !game.over && game.vehicles.length) {
				...
				// seta para a esquerda, A, ou arraste para a esquerda
				if ((e.keyCode && (e.keyCode == 37 || e.keyCode == 65)) || 
					(touch.hold && e.pageX < touch.x)) {
					playerVehicle.steer("left");
					hideTutorial();

				// seta para a direita, D ou arraste para a direita
